<?php
/**
 * class-isarud-hepsiburada.php — WP Plugin Hepsiburada Bridge Client
 * v6.6.3 - Hepsiburada entegrasyonu (Trendyol clone, 17 endpoint)
 *
 ...
 * - Hepsiburada Merchant ID/Username/Password ASLA WP'ye gelmez (sunucuda kalır)
 * - Tüm istekler isarud.com'a gider, Cloud Sync key ile auth
 */

if (!defined('ABSPATH')) exit;

class Isarud_Hepsiburada {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_isarud_hepsiburada_status',              [$this, 'ajax_status']);
        add_action('wp_ajax_isarud_hepsiburada_listings',            [$this, 'ajax_listings']);
        add_action('wp_ajax_isarud_hepsiburada_listing_delete',      [$this, 'ajax_listing_delete']);

        // Phase 2 - Stock & Price
        add_action('wp_ajax_isarud_hepsiburada_sync_stock',          [$this, 'ajax_sync_stock']);
        add_action('wp_ajax_isarud_hepsiburada_sync_single',         [$this, 'ajax_sync_single']);

        // Phase 3 - Categories
        add_action('wp_ajax_isarud_hepsiburada_categories',          [$this, 'ajax_categories']);
        add_action('wp_ajax_isarud_hepsiburada_category_attributes', [$this, 'ajax_category_attributes']);

        // Phase 4 - Orders
        add_action('wp_ajax_isarud_hepsiburada_orders',              [$this, 'ajax_orders']);
        add_action('wp_ajax_isarud_hepsiburada_order_pull',          [$this, 'ajax_order_pull']);
        add_action('wp_ajax_isarud_hepsiburada_package_status',      [$this, 'ajax_package_status']);
        add_action('wp_ajax_isarud_hepsiburada_package_tracking',    [$this, 'ajax_package_tracking']);

        // Phase 5 - Returns
        add_action('wp_ajax_isarud_hepsiburada_returns',             [$this, 'ajax_returns']);
        add_action('wp_ajax_isarud_hepsiburada_return_approve',      [$this, 'ajax_return_approve']);
        add_action('wp_ajax_isarud_hepsiburada_return_reject',       [$this, 'ajax_return_reject']);

        // Phase 6 - Connect Flow
        add_action('wp_ajax_isarud_hepsiburada_stores',              [$this, 'ajax_stores']);
        add_action('wp_ajax_isarud_hepsiburada_connect_url',         [$this, 'ajax_connect_url']);
    ...
    public function get_status(): array {
        return $this->bridge_request('hepsiburada/status');
    }

    public function get_listings(int $page = 0, int $size = 50, string $filter = 'all'): array {
        return $this->bridge_request('hepsiburada/listings', [
            'page' => $page,
            'size' => $size,
            'filter' => $filter,
        ]);
    ...
    public function delete_listing(string $merchantSku): array {
        return $this->bridge_request("hepsiburada/products/{$merchantSku}", [], 'DELETE');
    }
    ...
    public function sync_stock_bulk(array $items): array {
        return $this->bridge_request('hepsiburada/sync/stock', ['items' => $items], 'POST');
    }

    public function sync_stock_single(string $merchantSku, int $quantity, float $price, ?float $listPrice = null): array {
        $item = [
    ...
    public function get_categories(?int $parentId = null): array {
        ...
        return $this->bridge_request('hepsiburada/categories', $params);
    }

    public function get_category_attributes(int $categoryId): array {
        return $this->bridge_request("hepsiburada/categories/{$categoryId}/attributes");
    }
    ...
    public function get_orders(?string $status = null, int $days = 7, int $page = 0, int $size = 50): array {
        ...
        return $this->bridge_request('hepsiburada/orders', $params);
    }

    public function pull_orders(int $days = 7): array {
        return $this->bridge_request('hepsiburada/orders/pull', ['days' => $days], 'POST');
    }

    public function update_package_status(string $packageNumber, string $status): array {
        return $this->bridge_request("hepsiburada/orders/{$packageNumber}", ['status' => $status], 'PUT');
    }

    public function submit_tracking(string $packageNumber, string $trackingNumber, ?string $cargoCompany = null): array {
        $data = ['trackingNumber' => $trackingNumber];
        if ($cargoCompany !== null) {
            $data['cargoCompany'] = $cargoCompany;
        }
        return $this->bridge_request("hepsiburada/orders/{$packageNumber}", $data, 'PUT');
    }
    ...
    public function get_returns(int $page = 0, int $size = 50): array {
        return $this->bridge_request('hepsiburada/returns', [
            'page' => $page,
            'size' => $size,
        ]);
    }
    ...
    public function approve_return(string $claimId): array {
        return $this->bridge_request("hepsiburada/returns/{$claimId}", ['action' => 'approve'], 'POST');
    }
    ...
    public function reject_return(string $claimId, ?string $reason = null): array {
        ...
        return $this->bridge_request("hepsiburada/returns/{$claimId}", $data, 'POST');
    }
    ...
    private function check_ajax(): void {
        ...
        check_ajax_referer('isarud_hepsiburada_nonce', 'nonce');
    }
    ...
    public function ajax_listing_delete(): void {
        $this->check_ajax();
        $merchantSku = sanitize_text_field($_POST['merchant_sku'] ?? '');
        if (!$merchantSku) wp_send_json_error(['message' => 'merchant_sku gerekli']);
        wp_send_json($this->delete_listing($merchantSku));
    }
    ...
    public function ajax_sync_single(): void {
        ...
        if (!$merchantSku) wp_send_json_error(['message' => 'merchant_sku gerekli']);
        wp_send_json($this->sync_stock_single($merchantSku, $quantity, $price, $listPrice));
    }
    ...
    public function ajax_package_status(): void {
        $this->check_ajax();
        $packageNumber = sanitize_text_field($_POST['package_number'] ?? '');
        $status = sanitize_text_field($_POST['status'] ?? '');
        if (!$packageNumber || !$status) wp_send_json_error(['message' => 'package_number ve status gerekli']);
        wp_send_json($this->update_package_status($packageNumber, $status));
    }
    ...
    public function ajax_package_tracking(): void {
        ...
        if (!$packageNumber || !$trackingNumber) wp_send_json_error(['message' => 'package_number ve tracking_number gerekli']);
        wp_send_json($this->submit_tracking($packageNumber, $trackingNumber, $cargoCompany));
    }
    ...
    /**
     * Get user's stores
     * AJAX: isarud_hepsiburada_stores
     */
    ...
    /**
     * Get Hepsiburada connect URL for a specific store
     * AJAX: isarud_hepsiburada_connect_url
     */
    public function ajax_connect_url(): void {
        ...
        $return_url = admin_url('admin.php?page=isarud-hepsiburada');

        $response = wp_remote_post('https://isarud.com/api/v2/marketplace/hepsiburada/connect-url', [
            'timeout' => 30,
            'headers' => [
                'X-Marketplace-Key' => $api_key,
                'Accept' => 'application/json',
    ...
    /**
     * Show admin notice when ?isarud_hepsiburada_connected=1 query param exists
     * Hook: admin_notices
     */
    public function maybe_show_callback_notice(): void {
        if (!isset($_GET['isarud_hepsiburada_connected']) || $_GET['isarud_hepsiburada_connected'] != '1') {
            return;
        }
