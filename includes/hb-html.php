<?php
/**
 * hb-html.php — WP Plugin Hepsiburada Admin UI
 * v6.6.0 — 8-tab modern interface
 *
 * AJAX endpoint'leri Isarud_Hepsiburada class'ına bağlıdır.
...
<div class="wrap" id="isarud-hepsiburada-app">
    ...
    <div id="isarud-hb-loading" class="isarud-hb-loading">
        ...
        <?php _e('Checking Hepsiburada connection...', 'api-isarud'); ?>
    </div>
    ...
    <div id="isarud-hb-not-connected" class="isarud-hb-not-connected" style="display:none;">
        <div class="isarud-hb-empty" style="max-width:600px;margin:0 auto;">
            <h2 style="font-size:24px;margin-bottom:10px;">🛒 <?php _e('Connect to Hepsiburada', 'api-isarud'); ?></h2>
            <button type="button" class="button button-primary button-hero" id="isarud-hb-connect-btn" style="background:#ff6000;border-color:#ff6000;font-weight:600;font-size:16px;padding:12px 32px;height:auto;line-height:1.4">
                🔗 <?php _e('Connect with Hepsiburada', 'api-isarud'); ?>
            </button>
            <p style="color:#6b7280;margin-top:15px;font-size:13px;line-height:1.6;">
                <?php _e('You can integrate your store with the API Key, API Secret, and Seller (Current) Number that you will receive from the Hepsiburada Seller Panel.', 'api-isarud'); ?>
            </p>
        </div>
    </div>
    ...
    <div id="isarud-hb-connect-modal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.6);z-index:9999;align-items:center;justify-content:center;">
        <div style="background:#fff;border-radius:12px;max-width:520px;width:90%;padding:30px;box-shadow:0 10px 40px rgba(0,0,0,0.3);max-height:80vh;overflow-y:auto;">
            ...
            <p style="color:#666;font-size:14px;"><?php _e('Select your isarud store to connect with Hepsiburada:', 'api-isarud'); ?></p>
    ...
    <div id="isarud-hb-app" style="display:none;">
        ...
        <div class="isarud-hb-tab-content" data-tab-content="listings">
            <div style="display:flex;justify-content:space-between;align-items:center;margin:15px 0;">
                <h3><?php _e('Hepsiburada Products', 'api-isarud'); ?></h3>
                <div>
        ...
        <div class="isarud-hb-tab-content" data-tab-content="stock" style="display:none;">
            ...
            <div class="isarud-hb-card isarud-hb-card-blue">
                ...
                <p><?php _e('Send a product\'s barcode, stock, and price information to Hepsiburada.', 'api-isarud'); ?></p>
            ...
            <div class="isarud-hb-card isarud-hb-card-purple" style="margin-top:20px;">
                ...
                <p style="color:#666;font-size:13px;line-height:1.6;margin-bottom:15px;"><?php _e('When this feature is enabled, whenever you add a new product or update an existing product in WooCommerce, the product is automatically sent to all active marketplaces (including Hepsiburada).', 'api-isarud'); ?></p>
                ...
                <p style="color:#999;font-size:12px;margin-top:12px;line-height:1.5;">
                    ...
                    <strong><?php _e('Hepsiburada category and brand matching', 'api-isarud'); ?></strong>
                    <?php _e('completed. Otherwise, the export will fail.', 'api-isarud'); ?>
                </p>
            </div>

            <div class="isarud-hb-card" style="margin-top:20px;">
                <h4>📦 <?php _e('Manual Bulk Send (WooCommerce → Hepsiburada)', 'api-isarud'); ?></h4>
                <p style="color:#666;font-size:13px;line-height:1.6;margin-bottom:15px;"><?php _e('Use this button to send all your WooCommerce products to Hepsiburada at once. Only products with completed category and brand matching will be transferred.', 'api-isarud'); ?></p>
                <button class="button button-primary" id="isarud-hb-bulk-export-btn" style="background:#ff6000;border-color:#ff6000;font-weight:600;">
                    🚀 <?php _e('Send All Products to Hepsiburada', 'api-isarud'); ?>
                </button>
        ...
        <div class="isarud-hb-tab-content" data-tab-content="cargo" style="display:none;">
            ...
            <p><?php _e('Add shipping tracking number to Hepsiburada orders.', 'api-isarud'); ?></p>
            <div class="isarud-hb-card isarud-hb-card-blue">
        ...
        <div class="isarud-hb-tab-content" data-tab-content="settings" style="display:none;">
            ...
            <div class="isarud-hb-card">
                <h4><?php _e('Hepsiburada Connection Information', 'api-isarud'); ?></h4>
                <p><?php _e('Status and information of your Hepsiburada API connection.', 'api-isarud'); ?></p>
                <table class="form-table">
                    <tr>
...
jQuery(function($){
    ...
    function loadListings() {
        ...
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_listings',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
            page: 0,
            size: 50,
    ...
    $(document).on('click', '.isarud-hb-toggle', function(){
        ...
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_listing_' + action,
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
            barcode: barcode
        }, function(r){
    ...
    $(document).on('click', '.isarud-hb-delete', function(){
        var barcode = $(this).data('barcode');
        if (!confirm('<?php echo esc_js(__('Delete this product from Hepsiburada? This cannot be undone.','api-isarud')); ?>')) return;
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_listing_delete',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
            barcode: barcode
        }, function(r){
            alert(r?.message || (r?.success ? 'OK' : 'Hata'));
    ...
    $('#isarud-hb-single-sync').on('click', function(){
        var data = {
            action: 'isarud_hepsiburada_sync_single',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
    ...
    function loadOrders() {
        ...
        area.html('<p><?php echo esc_js(__('Loading...','api-isarud')); ?></p>');
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_orders',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
    ...
    $(document).on('click', '.isarud-hb-pkg-status', function(){
        ...
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_package_status',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
            package_id: pkg,
            status: st
        }, function(r){
    ...
    function loadClaims() {
        ...
        area.html('<p><?php echo esc_js(__('Loading...','api-isarud')); ?></p>');
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_returns',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>'
        }, function(r){
            if (!r?.success) { area.html('<p>' + (r?.message || 'Hata') + '</p>'); return; }
    ...
    $(document).on('click', '.isarud-hb-claim-approve', function(){
        ...
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_return_approve',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
            claim_id: cid,
            line_ids: lines
        }, function(r){
    ...
    $('#isarud-hb-connect-btn').on('click', function(){
        ...
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_stores',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>'
        }, function(r){
    ...
    $(document).on('click', '.isarud-hb-store-item', function(){
        ...
        $item.css({background:'#fef3e8', borderColor:'#ff6000'});

        // Connect URL al ve yeni tab'da aç
        $.post(ajaxurl, {
            action: 'isarud_hepsiburada_connect_url',
            nonce: '<?php echo wp_create_nonce("isarud_hepsiburada_nonce"); ?>',
            store_id: storeId
        }, function(r){
    ...
    // ═══════ BULK EXPORT (WC → Hepsiburada) ═══════
    $('#isarud-hb-bulk-export-btn').on('click', function(){
        if (!confirm('<?php echo esc_js(__('All WooCommerce products will be sent to Hepsiburada. Continue?', 'api-isarud')); ?>')) return;
        ...
        $.post(ajaxurl, {
            ...
            $btn.prop('disabled', false).html('🚀 <?php echo esc_js(__('Send All Products to Hepsiburada', 'api-isarud')); ?>');
            if (r && r.success) {
